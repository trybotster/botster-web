...
assert.deepEqual(kinds("const realSetTimeout = setTimeout;\nglobalThis.setTimeout = (callback) => callback();\nrealSetTimeout(done, 5);"), [], "timer captures are not calls");
for (const polling of ["await page.waitForTimeout(500);", "await page.waitForFunction(() => true);", "await row.waitFor();", "await page.waitForURL(/x/);", 'await page.goto(url, { waitUntil: "networkidle" });']) {
