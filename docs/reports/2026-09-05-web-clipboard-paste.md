...
- Live lane: run 2 confirmed the boundary fix and surfaced the Core `stale_mode` placeholder gap. A real-Hub paste is blocked on the corrected Core candidate that supplies the authoritative token on `stale_mode`, not on a Web workaround. It also lacks bracket-off and bracket-on, Unicode, CRLF, and cancellation or error cases. Local success does not establish live acceptance.
