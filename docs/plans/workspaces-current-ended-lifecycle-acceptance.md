...
- A public shutdown transitions one referenced session current -> ended in the DOM without a new `plugin_surface_render`, `list_sessions`, or imperative refresh; the ended row must become visibly legible in its semantic region and the old current-region row must disappear.
