...
- The installed `@trybotster/ui-contract@0.2.0` admits a bound `id` only on the direct `UiBindList.item_template` root, where row context exists. `IonicUiNodeRenderer.tsx` realizes that root identity, drops empty/non-string identities, and filters colliding realized identities. Distinct bound ids on descendants are not part of the current contract.
...
- No descendant-id synthesis, client-local id rewriting, or assertion that requires distinct bound `item_template` descendant ids. Existing `c78f3bf` ids such as `botster-workspaces-session-label-<uuid>` and `botster-workspaces-remove-<uuid>` are artifacts of Lua statically expanding one node tree per reference; an entity-bound surface cannot reproduce those distinct descendant ids under UI Contract 0.2.0.
...
- **Hub descendant identity constraint:** open Hub ticket `ticket_1785443253_376782` on target `tgt_7e208a0c76a44980a83b63af976b1f22` owns any contract expansion for distinct bound identities on `BindList` descendants. This plan deliberately needs only the 0.2.0 direct row-root identity, so that ticket is not a blocker. If implementation proves descendant identity is necessary, stop and register the prerequisite against the Hub target rather than inventing a Web or Workspaces workaround.
...
- Contract constraint: repeated session rows expose stable identity only on the direct bound `item_template` root. The Web runner may use static region-container ids and must find row labels/badges/actions by containment; distinct descendant node ids are neither assumed nor accepted as a prerequisite.
...
- **Identity-grammar overreach:** future producer work could be asked for distinct bound descendant ids that UI Contract 0.2.0 forbids. Mitigation: select only the bound direct row root, scope descendants by containment, and route any proven need for richer identity to Hub ticket `ticket_1785443253_376782`.
...
- Renders canonical current and ended references through the delivered owner tree and production registry/store, identifying each repeated row by its bound direct `item_template` root and scoping labels/badges/actions by containment.
...
- Preserve `question_1785545318_550751` and its answer as a binding assumption in Plan, gate evidence, and downstream handoff.
