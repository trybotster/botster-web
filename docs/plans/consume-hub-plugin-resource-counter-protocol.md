...
- Repository authority inspected: `README.md`, `docs/architecture.md`, `package.json`, `package-lock.json`, `scripts/check-daemon-protocol-drift.mjs`, `src/botster/generated/daemon-protocol.ts`, `src/botster/realHubDaemonDto.ts`, `src/botster/__fixtures__/generatedDaemonProtocol.ts`, `src/botster/__fixtures__/uiNodeConformance.ts`, `src/botster/IonicUiNodeRenderer.tsx`, `src/botster/uiNodes.ts`, `src/botster/UiNodeSurface.tsx`, `src/botster/uiPresentation.ts`, `src/botster/client.ts`, `src/App.tsx`, `src/App.test.mjs`, and prior protocol-consumption plans under `docs/plans/`.
- Upstream evidence: public `@trybotster/hub-test-support@0.1.18` was independently installed from the normal registry with integrity `sha512-llWRLNjFVKzi3CxII1waPO6u2w+TEAesiI8esVTEu8/rwuedeoZXXYTD0Sew76/8reysoZ1gy4P6LNoTptXsTA==`; its daemon protocol SHA-256 is `956b2ce7c07523af848da885006c21f944b580799551b208f59f96450a245c0b` and contains `plugin_resource_counters`. Published `0.1.17` is immutable but stale and must not be used. The matching Hub source is not on `origin/main`: it is pinned to `origin/project-pipelines/ticket_1785199716_875648` at `c8e719e0a067a904221d90346db500efec7dd215` (open PR #185). Hub `origin/main` at planning time is `822a75af9c9cc1815a2aaff18f3294d82810fd1f`, whose protocol hashes to `ce28d2ada7f80845ff4c8afb47e689bd80b356f1d2fbb103559589f5091ab157` and contains no counter field.
- Project target inventory: Project Pipelines now registers this Web target on `project_1785199669_548853` as `project_target_1785530278_216405`, alongside the existing Core and Hub targets.
...
- The open Hub integration ticket `ticket_1785199716_875648` owns both sides of this seam: upstream unmerged protocol source/publication on open PR #185 and downstream seven-repository four-package proof. Its dependency `dependency_1785515833_230748` already makes that Hub ticket depend on this Web ticket. Do not add the reverse Web-to-Hub ticket edge: that would create a dependency cycle and prevent the already-published consumer update from unblocking the Hub run.
- The immutable, independently verified public `0.1.18` package is accepted as the executable Web prerequisite even though its source branch has not merged. Source corroboration and the Hub artifact gate must use the exact pinned branch commit, never ambient Hub `main`. The Web pin remains provisional relative to Hub source until PR #185 merges.
- This run must hand back the Web commit, hashes, pinned Hub ref, and provisional-source status to `ticket_1785199716_875648`; it must not broaden into that Hub-owned workload. If the installed public coordinate, pinned source artifact, or vendored bytes disagree, stop and route the release/synchronization defect to Hub target `tgt_7e208a0c76a44980a83b63af976b1f22` rather than repairing Hub-owned bytes here.
...
- The exact source corroboration ref is `origin/project-pipelines/ticket_1785199716_875648` at `c8e719e0a067a904221d90346db500efec7dd215`. Implement must fetch that ref immediately before comparison and fail if it moved rather than treating an amended open branch as the planned authority.
...
- Known consumer: `src/App.tsx` constructs outgoing `UiActionRequest.node_id` from `dispatch.node.id`, so the renderer/action dispatch type must guarantee a realized literal `UiNodeId` before that production entry point. Unknown until the clean compiler run: whether audited sibling consumers need additional narrow adaptations. Add only compiler- or conformance-proven changes and record each in the implementation artifact.
...
- `src/botster/__fixtures__/uiNodeConformance.ts`: compose canonical `bound_row_identity` into `uiNodeConformanceSnapshot` and add `/session` entity rows whose `lifecycle_class: "current"` satisfies its `where` filter, so the fixture reaches the real renderer with multiple bound identities.
- `src/App.test.mjs`: support metadata/revision/hash and token checks, typed counter fixture assertion, unchanged dependency-equality invariant, canonical `bound_row_identity` renderer/action proof, and unresolved/blank/duplicate identity negatives.
- `src/botster/IonicUiNodeRenderer.tsx`: expected narrow `UiAuthoredNodeId` realization at bind-list expansion and literal-id enforcement before React or action state.
- `src/botster/uiNodes.ts`: expected realized-node/action-dispatch typing so downstream action consumers receive a literal `UiNodeId`, not the authored `UiNodeId | UiBind` union.
- `src/App.tsx`: production plugin-surface dispatch site; preserve a plain literal `node_id` in `UiActionRequest` after renderer realization and prove no binding object can cross the transport boundary.
- `src/botster/UiNodeSurface.tsx`, `src/botster/uiPresentation.ts`, and `src/botster/client.ts`: explicit compiler/consumer audit surfaces, expected unchanged unless the `0.2.0` types prove a narrow adaptation is required.
...
- Unmerged source can strand Web: PR #185 may revise the generated protocol again after immutable `0.1.18` publication, leaving Web green against its package while divergent from eventual Hub main. Treat the pin as provisional, record the merge/follow-up obligation, and require a new Web re-vendor ticket if the merged source hash differs.
...
3. Fetch Hub `origin/project-pipelines/ticket_1785199716_875648`, require it still resolves exactly to `c8e719e0a067a904221d90346db500efec7dd215`, and record that Hub `origin/main` remains at the separately observed hash/ref. Require SHA-256 `956b2ce7c07523af848da885006c21f944b580799551b208f59f96450a245c0b` for installed, vendored, and pinned-branch daemon protocol bytes. A supplied `BOTSTER_HUB_CLIENT_DAEMON_PROTOCOL` comparison may not skip. Do not substitute Hub main while it lacks the field.
...
7. Run the Hub-owned `production-package-runtime-evidence artifact-gate` with the Hub repository detached at pinned commit `c8e719e0a067a904221d90346db500efec7dd215`, this Web worktree, and a fresh installed `0.1.18` package. Its declared, installed, metadata, source, vendored, and conformance-revision checks must all be true. Record that the same gate is expected to fail against current Hub main because main does not yet own these bytes; do not misclassify that known divergence as a package defect.
8. Handoff downstream proof to `ticket_1785199716_875648`: record the final Web commit, package integrity/protocol hashes, pinned Hub branch commit, and that Web's coordinate is provisional until PR #185 merges. If PR #185 has merged by Verify, rerun the artifact gate against fetched Hub main. If it has not, the Hub ticket owns the post-merge equality check and must open a follow-up Web re-vendor ticket if the merged protocol hash differs from `956b2ce7...`. The full seven-repository `script/test-production-package-runtime` and four-package workload remain required downstream proof but are not implemented or altered here.
