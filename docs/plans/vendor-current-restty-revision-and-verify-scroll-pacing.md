...
report.
W2. The mounted handler report count and the PTY report count agree, with
application mouse tracking on.
W3. The mounted handler scroll direction and the PTY scroll direction agree,
with application mouse tracking on.
...
terminal-render tests. W1 through W11 cover wheel behavior and the PTY report
count and direction. G6 covers retained GHOSTSNP scrollback, but its fixture
retains only `SCROLLBACK-LINE-000` through `SCROLLBACK-LINE-029`. G7 delivers
one PAGE frame and does not prove a large history. G8 closes that gap.
...
- Mouse tracking: off for the whole lane.
- Workload: the frozen large-history workload already in the repository,
  `fixtures/terminal-baseline/history-seed.sh`. It is 400 lines of 80 bytes, and
  each line is its own zero-padded index. Feed those bytes through the harness
  `emitOutput` sink, which is the live terminal output path that reaches the
  mounted renderer. G8 makes no incremental-reader claim, because `emitOutput`
  reaches the live `subscribeOutput` listener and not
  `bindIncrementalSnapshotReader`. G7 owns incremental reader proof.
...
- Assertions: the settled viewport ends at line 400; a known wheel distance
  ...
  every visible line keeps its full 80 bytes; and the PTY input sink receives no
  wheel bytes for the whole lane.
- Failure condition: a missing line, a truncated line, a rendered scroll
  distance that disagrees with the dispatched wheel distance, any PTY wheel
  byte, or any page error.
...
| R10 | The large-history gate drifts into a timing claim. | G8 asserts rendered rows only. It records no wall-clock value. |
| R11 | A gate asserts both Restty wheel destinations for one event. | `onWheel` routes each event to one destination. G8 runs with mouse tracking off and requires no PTY wheel bytes. W2 and W3 run with tracking on. |
...
## 15. Plan Review response, revision 3

Plan Review `review_1787763495_429246` returned `changes_required` with two
findings. This revision answers both.

- `finding_1787763495_644751` (product, high). The finding is correct. I
  verified `createPointerAuxHandlers.onWheel` at `cd1911d0f` in
  `src/runtime/create-runtime/interaction-runtime/bind-pointer-aux-handlers.ts`:
  active application mouse reporting sends the report, prevents the default, and
  returns before `scrollViewportByWheel`, so one wheel event reaches one
  destination only. Revision 2 asked G8 to observe both at once, and the mounted
  smoke has no application process that could redraw rows from PTY reports.
  Revision 3 splits the proof. G8 is now a large-history local-scrollback lane
  with mouse tracking off, and it requires zero PTY wheel bytes. W2 and W3 keep
  the PTY report count and direction proof with tracking on. Revision 3 also
  removes the incremental-reader claim from G8. New risk row R11 records the
  one-destination rule.
- `finding_1787763495_147693` (process, info). Agreed, and this is an engine
  projection issue rather than a plan defect. Plan gate
  `gate_result_1787763190_569871` passed with the complete evidence, and its
  summary names the artifact and the checklist. The later `step.completed` event
  still projects an empty evidence object. Plan preserves the complete gate
  evidence on every submission and cannot write that projection.

## 16. Plan Review response, revision 4

Plan Review `review_1787763939_886143` returned `changes_required` with one
finding.

- `finding_1787763939_361066` (process, high). The finding is correct. My
  revision 3 edit script computed a reversed slice, which produced an empty
  search string. The replacement then inserted the new G8 text between every
  character of the file, so commit `43a772c` grew the plan to 1,022,627 lines
  and 58,410,241 bytes and destroyed the plan structure. Revision 4 restores the
  revision 2 file from commit `b11026e`, applies the G8 split, the
  incremental-reader correction, the W2 and W3 tracking note, risk row R11, and
  this response one time, and verifies the line count, byte count, headings, and
  diff before submission. The revision 3 product decisions are unchanged.
