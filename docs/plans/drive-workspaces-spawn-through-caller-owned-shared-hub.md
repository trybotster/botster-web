...
- Each driver summary is parseable structured output and cross-references every assigned case exactly once. The coordinator summary correlates both generations, including entry state, chosen rendered create control, prior-state observation, and case-assignment digest. Per-run provenance identifies the Hub binary and session-worker binary (path, version, source manifest), Workspaces package (path, manifest version/source kind, Git commit), and Web commit/build. Failure output retains the failing generation/case, last action/result/entity chronology, surface/list counts, route, provenance, and bounded realized node identities.
