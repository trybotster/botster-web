...
- Project Pipelines current context supplied the authoritative ticket, run, active Plan step and gate, target ID, artifacts, checklists, dependency/question state, Plan Review `review_1785605529_739903`, and its five returned findings. This revision corrects the false live-baseline premise, assigns an in-run coordinator, makes live proof unconditional, adds runtime provenance, and forbids authored-ID prefix reconstruction.
...
10. Add a repository-owned, test-only coordinator that reuses the setup shape from `scripts/live-caller-owned-repeatability.mjs`: create one fresh short data directory, start one Hub, install/enable Web and Workspaces once, create/admit real managed-Git targets/templates/base refs, launch `botster-web/web-client` through current Hub commands, pass the shared data directory and exact case-assignment JSON to the lifecycle-free driver, assert the driver's structured result, then own shutdown and fixture cleanup. This coordinator exists solely to prove the driver in this run; the final Workspaces parent replaces it downstream.
11. Reuse or extract pure helpers from the existing live harness only where both scenarios share mechanics. Keep the current standalone live, Workspaces compatibility/lifecycle, caller-repeatability, contract-matrix, and durable modes behaviorally unchanged.
12. Add deterministic script/helper tests, package-script wiring, and README documentation for both the direct driver contract and the mandatory repository-owned smoke command, ownership boundary, structured evidence/provenance shape, and downstream proof command.
...
- `scripts/workspaces-shared-hub-browser-driver.mjs` (new): narrow caller-owned production-browser executable; public app discovery, Playwright flow, case correlation, reconciliation proof, and structured output.
...
1. Lock deterministic failing tests around the missing caller-owned driver contract and in-run executor: the driver must reject absent shared Hub/app/case inputs and expose no Hub start/shutdown/package install/enable path; the smoke must fail unless the real driver completion ledger proves every assigned case executed.
...
3. Implement the new driver boundary: validate required `BOTSTER_LIVE_DATA_DIR` and `BOTSTER_WORKSPACES_SPAWN_CASES`, protocol-handshake the exact socket, read the already running app row, require successful health and matching structured local URL, launch Chromium, and install bounded event capture.
...
- With the same binaries, `npm run smoke:live-packaged-protocol:caller-repeatability` exits 0 and proves restored enabled-package reuse.
...
- The driver's final summary is parseable structured output and cross-references every assigned case exactly once. Its per-run provenance identifies the Hub binary and session-worker binary (path, version, source manifest), Workspaces package (path, manifest version/source kind, Git commit), Web commit/build, and case-assignment digest. Failure output retains the failing case, last action/result/entity chronology, surface/list counts, route, provenance, and bounded realized node identities.
