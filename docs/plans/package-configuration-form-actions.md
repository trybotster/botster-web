...
- Pipeline context: ticket `ticket_1782795779_951297`, run `run_1782795791_997993`, active step `botster_plan`, gate `botster_plan_gate`. Initial Plan Review `review_1782796267_271124` returned three findings: missing real dev-stack config-schema package, ambiguous live-hub fallback, and unstated TUI parity. This revision registers dependency `dependency_1782796372_772764` on hub ticket `ticket_1782796365_386145` and addresses all three findings.
...
- Real-package baseline: this botster-web repo does not contain a real dev-stack package with an authoritative configuration schema. Repo inspection found only fake/local configuration fixtures in `scripts/packaged-browser-smoke.mjs` and test fixtures. The required real proof is now registered as blocking hub dependency `ticket_1782796365_386145` ("Provide real dev-stack package configuration schema acceptance target"), which must add or designate a real local hub package exposing `DaemonPackage.configuration.schema`, one known successful value, one known invalid value, and refreshed persisted config state.
- Botster layers touched: React/Ionic SPA, browser real-hub adapter only if a narrow request/response gap is found, packaged browser smoke, live packaged harness, tests, and the registered hub/dev-stack dependency above. This ticket's botster-web implementation must not claim acceptance until that dependency is closed or an equivalent concrete real package id/field/invalid-case is documented in pipeline evidence.
- TUI parity: N/A for this ticket. The changed user surface is the botster-web React/Ionic package settings modal. Shared hub DTO/action behavior must remain client-neutral, but adding a TUI package configuration form is outside this ticket.
...
- No passing acceptance on deterministic browser/transport tests alone. Those tests are useful regression coverage, but the ticket cannot advance as implemented/verified without the real local dev-stack hub package round trip.
- No botster-tui implementation. TUI parity is explicitly out of scope because this ticket fixes a botster-web settings modal path.
...
- Blocking dependency: `ticket_1782796365_386145` must provide or designate the concrete real dev-stack hub package for live acceptance. Until that dependency is closed, the botster-web implementation can proceed only as code/test preparation; it cannot satisfy final acceptance.
- Unknown until dependency closure: the exact package id, field id/type, successful value, invalid value, and validation diagnostic shape. The dependency ticket is required to produce those details in hub/dev-stack evidence before this ticket's live harness can be completed.
...
- Unknown: whether the local pipeline environment has compatible `BOTSTER_HUB_BIN` and `BOTSTER_SESSION_WORKER_BIN` for live acceptance. If unavailable when the dependency package exists, the run is BLOCKED for environment evidence; deterministic browser/transport tests remain necessary but are never sufficient for passing this ticket.
...
- Blocking hub/dev-stack dependency: `ticket_1782796365_386145`, target `botster-hub`, must provide the real config-schema package acceptance target before final verification.
...
- Missing real package risk: without `ticket_1782796365_386145`, there is no concrete real dev-stack package in this repo to satisfy the ticket's required live proof. Treat that as blocked acceptance, not a passing fallback.
...
- Hard-gate precondition:
  - Dependency `ticket_1782796365_386145` must be closed, or equivalent pipeline evidence must name the concrete real dev-stack package id, configuration field id/type, successful value, invalid value, and hub-returned validation diagnostic shape.
  - If this precondition is unmet, Implement/Verify must mark the ticket blocked. Deterministic tests and local fixtures cannot satisfy acceptance by themselves.
- Real hub acceptance with the dependency-provided local dev-stack package:
  ...
  - If required hub binaries or session-worker binaries are unavailable in the execution environment after the dependency package exists, the correct result is BLOCKED with exact missing-binary/protocol evidence, not a deterministic-only pass.
...
- Capture a hub/dev-stack note when dependency `ticket_1782796365_386145` establishes the real config-schema package used for client acceptance.
