...
1. Ended-session presentation path.
   ...
   - `scripts/live-packaged-protocol-harness.mjs` `assertDurableSeededSessionsVisible`: for each seeded id, require a `hub_frame` session record with `lifecycle_class: "ended"` through `waitForHarnessEvent`, then require the row inside `dashboard-ended-sessions`, and require zero matches inside the current `Sessions` list. `proveExternalSessionLifecycle` in durable mode: open Diagnostics before the existing cap check (this function now runs after terminal attach, so Diagnostics is not already visible), then after the `exited` patch, require the external row inside `dashboard-ended-sessions` before `remove_session`, and keep the existing `detached` wait.
...
- `proveExternalSessionLifecycle` currently runs after terminal attach, so the page is not on Diagnostics. The Implementer opens Diagnostics before the durable cap check, then `openHomeView` before the ended-section wait. The cap check itself stays unchanged.
