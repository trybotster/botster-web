...
- Required negative-proof placement: generic-action rejection belongs only where it can go red—before semantic selection on a production-rendered generic-action fixture, and after transport capture in `assertTwoGenerationLedger`. Do not add a tautological `!== botster_workspaces.open` assertion downstream of a selector that already pins `open_spawn`.
...
- `src/App.test.mjs`: render the real Ionic button primitive twice with identical opaque node/action/payload and different visible labels; prove both match the exact exported selector contract, a generic-action/old-dialog fixture does not, the ledger rejects captured generic identity, and the `/^Spawn$/` source path is absent.
...
4. Add a bounded `spawn_opener` record to each case summary containing DOM read-back identity, the captured request action/node/payload, and matching accepted result. Strengthen `assertTwoGenerationLedger` so all cold/reused cases require `botster_workspaces.open_spawn`, exact node identity across DOM/request/result, the existing payload shape, and existing reconciliation evidence. A mutated summary using `botster_workspaces.open` must fail here because ledger values come from captured transport rather than the selector.
5. Delete the visible-copy selector and add a source guard proving `filter({ hasText: /^Spawn$/ })` is absent. Keep this as a backstop to the production-renderer/selector execution, not a substitute. Record two ablations: restoring the copy filter must fail the source guard, and swapping the rendered/captured identity to generic open must fail the selector fixture and ledger test.
...
- **Deprecated-path compatibility leak:** allowing `botster_workspaces.open` with dialog payload would preserve ambiguity. Mitigation: fail exact identity validation and ledger evidence unless the opener request/result use `open_spawn`; retain no fallback.
- **Tautological negative assertion:** checking `!= botster_workspaces.open` after an exact `open_spawn` selector can never fail and repeats a defect caught in producer review. Mitigation: reject generic identity only in the pre-selection production-renderer fixture and post-capture ledger, never downstream of the pinning predicate.
...
- `assertTwoGenerationLedger` rejects missing opener evidence, any captured opener request/result using generic `botster_workspaces.open`, mismatched opaque node identity, or missing producer payload correlation.
- Source guards prove the live harness imports the shared selector and the old `filter({ hasText: /^Spawn$/ })` path is absent.
- Ablations are explicit: restoring the old copy filter fails the source guard; changing the rendered/captured semantic identity to generic open fails the production-renderer selector fixture and ledger test. No negative assertion is accepted after a predicate that already pins the same value.
...
- `finding_1785618794_563466` (high): removed the dead post-selector generic-action assertion. Generic rejection now lives only in a pre-selection production-renderer fixture and the post-capture two-generation ledger, where each mutation can independently turn the test red.
