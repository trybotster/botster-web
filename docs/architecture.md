...
`@trybotster/ui-contract@0.3.3`. Test assets come from `@trybotster/hub-test-support@0.1.45` (host protocol 9, conformance revision-49), vendored verbatim from Hub 0b9a11d9 into `test-support/hub-test-support` as a `file:` dev dependency until Hub publishes it. Host DTOs come from the Hub-generated `daemon-protocol.ts` and Core terminal
