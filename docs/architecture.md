...
The production package transport is `src/botster/webrtcDaemonClient.ts`. It consumes the generated `DaemonLocalWebrtcBootstrap`, sends a generated `local_webrtc_signal` daemon request with the browser SDP offer, and opens an ordered `RTCDataChannel`. Every response frame is a generated `DaemonLocalWebrtcResponseChunk`; the browser sparsely reassembles a complete byte-exact JSON `AesGcmEnvelope` before decryption. One-chunk and multi-chunk responses use that sole path—there is no legacy raw-envelope decoder or negotiation branch. Reassembly is generation- and pending-request-bound, limited to frames below 65,536 UTF-8 bytes, responses through 16 MiB, 16 concurrent assemblies, and 32 MiB of logical aggregate retention including bookkeeping. Timeout, malformed, conflicting, stale, or over-budget state resets the peer generation so positional FIFO cannot slide onto a later request. The encrypted plaintext remains the generated `DaemonRequest` / `DaemonResponse` protocol, including terminal attach, drain, resize, input, process-exit, and shutdown frames. The checked TypeScript DTO artifact in `src/botster/generated/daemon-protocol.ts` is copied from the authoritative hub-client generated artifact; WebRTC DTOs are not maintained by hand in botster-web.

The packaged live protocol harness defaults to WebRTC. Across two same-URL reload cycles it renews the WebRTC bootstrap, explicitly attaches to the existing session, and records one exact subscription's daemon events to assert `Attaching -> optional opaque Snapshot/Scrollback -> Attached -> live terminal output`. Snapshot/Scrollback carry revision-14 `payload_base64`, literal `payload_encoding: "base64"`, and byte counts; they are validated as opaque metadata and never written into Restty. When `Attached` arrives, `RealHubTerminalDataPlane` automatically requests ReadScreen, buffers later live terminal output, writes non-empty screen text first, then flushes that buffer FIFO. The hydration state belongs to one attachment generation, so detach or replacement discards stale replies and buffered output. The harness separately proves metadata chronology and renderer-visible restoration through the shipped `dogfoodMode -> TerminalViewHost -> DefaultTerminalViewBridge -> RealHubTerminalDataPlane -> Restty` path. It records redacted response-chunk telemetry and binary provenance while retaining keyboard, resize, refresh, exit, and cleanup proof. `capture_snapshot` remains an explicit mounted metadata control; it is not visible-history restoration.
...
Compatibility diagnostics require the existing functional daemon features plus `terminal_readback`, and the hand-owned minimum conformance fixture revision is 14. The published `@trybotster/hub-test-support@0.1.8` artifact provides the authoritative generated DTOs and conformance revision 15 fixtures. A hub below revision 14 is rejected with an explicit observed-versus-required diagnostic even if it advertises readback, because revisions 12/13 expose the superseded renderable-history contract. Hub-emitted `unsupported_feature` or `compatibility_mismatch` diagnostics carried on the hub-status record still take precedence over locally synthesized compatibility rows.
