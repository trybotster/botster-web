...
- The connection diagnostics panel starts with the selected transport mode and adds targeted rows when local WebRTC, the bridge fallback, the control stream, daemon schema, compatibility descriptor, operator/action response, or terminal stream fails.
