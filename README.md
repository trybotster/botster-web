...
This command builds the app, starts the package bridge in real-hub mode, starts an isolated local hub when `BOTSTER_HUB_BIN` is supplied, opens the compiled packaged UI in Playwright Chromium, and drives the mounted terminal data-plane path. Its default transport is the production WebRTC data plane. It proves status/schema compatibility, package listing, session listing, spawn of `botster-web-dogfood-session`, two same-URL browser refresh and WebRTC reconnect cycles followed by explicit Attach to the existing session, historical output restored from `snapshot.data` or `scrollback.data` into Restty, continued live terminal behavior, browser keyboard input through the mounted Restty renderer producing exactly one `send_input` and echo output containing `botster-web-dogfood-echo:<input>`, explicit `read_screen` and `capture_snapshot` round trips through the Playwright-injected mounted controls, resize output containing `botster-web-dogfood-size:<rows>x<cols>`, deterministic exit through `botster-web-dogfood-exit`, observed `process_exit`, and clean shutdown. For the initial mount and each refresh attach, it waits for `Attached`, sends a unique live terminal probe, records the exact subscription, and asserts `Attaching -> optional non-empty Snapshot/Scrollback -> Attached -> live terminal output`; opaque positive snapshot byte counts without non-empty `data` are diagnostic metadata, not restored terminal history. The harness also records the resolved hub and session-worker binary paths and, when discoverable from their owning Cargo manifests, their package versions; the observed subscription chronology is the readiness-order proof. Each WebRTC history restore must report attach/drain response-chunk telemetry with `total_bytes`, `chunk_count`, and `duration_ms`; duration must remain below the 10-second request deadline and at or below 8 seconds to preserve 2 seconds of headroom. Release acceptance repeats the full command five times and records all five telemetry summaries. Published conformance-fixture tests prove large response byte exactness in this repo; Hub transport tests own the concrete above-DataChannel-message-limit chunking proof, and the live attach-readiness assertion owns the production subscription chronology proof.

Attach/Drain history is available on the shipped path today. The published `@trybotster/hub-test-support@0.1.5` artifact supplies the generated readback and response-chunk DTOs plus revision-12 conformance fixtures used by deterministic tests. Typed `read_screen` text and `capture_snapshot` metadata helpers are exposed only through the Playwright-injected live-harness controls; shipped browser sessions do not install or invoke them. Snapshot metadata (`rows`, `cols`, optional `payload_format`, and `payload_bytes`) is not a renderable terminal byte stream. `terminal_readback` therefore remains optional with a warning when absent, and the hand-owned minimum compatible conformance revision remains 1.
