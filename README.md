...
The published `@trybotster/hub-test-support@0.1.7` artifact supplies the generated binary-safe history DTOs, typed readback/response-chunk contracts, and revision-14 conformance fixtures used by deterministic tests. Shipped browser sessions invoke `read_screen` automatically when their exact subscription reaches `Attached`; live output received during that request is buffered FIFO so restoration cannot overwrite newer bytes. Empty screen text fabricates no history, and stale replies from detached or replaced attachment generations are discarded. `capture_snapshot` remains an explicit metadata helper and its opaque payload metadata is not a renderable terminal byte stream. Compatible hubs must advertise `terminal_readback` and conformance fixture revision 14 or newer.
