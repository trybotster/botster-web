...
- Host-control v9 `ClientFrame` requests carry a monotonic decimal `request_id`; encrypted `ServerFrame` responses, host events, and typed close reasons travel over the ordered control DataChannel after its Hello. Web keeps at most 32 requests outstanding.
- Each entity or package-event subscription uses one separate ordered DataChannel. Web creates it only after Hub returns an opaque reserved label and generation. The channel sends its own encrypted Hello before it carries entity frames or package events.
- Each terminal subscription uses one separate ordered DataChannel. Web creates it only after Hub reserves an opaque label and generation. The channel sends its own encrypted Hello, then carries binary chunks: Core scheme 2 terminal bodies toward Web and Core input frames toward Hub, AES-GCM sealed per chunk with no JSON or base64.
...
- `@trybotster/ui-contract@0.3.3` is the sole `UiNode`, action request/result, package-surface descriptor, supported-operation, entity-options projection, manifest-navigation, and package notice-reaction vocabulary source. Hub owns package admission and projects sanitized package/navigation rows plus `notice_reactions`. Host DTOs come from the Hub-generated `daemon-protocol.ts`; Core terminal codecs, key tables, and feature tokens come from the Core-generated `terminal-protocol.ts`. Both are copied verbatim into `src/botster/generated/`. Test assets come from `@trybotster/hub-test-support@0.1.44` (host protocol 9, conformance revision-49), vendored verbatim from Hub 8cdab71 into `test-support/hub-test-support` as a `file:` dev dependency until Hub publishes it, and from `@trybotster/terminal-protocol@0.3.0`.
...
- Restty is the terminal renderer only. It imports GHOSTSNP snapshots and receives live bytes through the WebRTC-backed data plane; it does not own terminal truth, answer OSC color queries (`readOnly`), or send input. Keyboard, IME, pointer, wheel, focus, resize, and paste are captured at the terminal container as semantic input and encoded by the worker's Ghostty; every outcome is reported to the user.
...
Each keep-alive pass must print `live-shared-session-terminal-lane`, `live-shared-session-cancel-passed`, and `live-shared-session-keep-alive-passed` for the same session id. Each keep-alive pass returns the producer to the primary screen before it prints the keep-alive marker. The exit pass must print `live-shared-session-exit-passed`. Page reload is not reconnect on this lane; reconnect proof closes the DataChannel on the surviving document. The cancel pass holds no attach with a hook. It navigates Home as soon as the new attach request is recorded. The marker field `pending_attach_cancellation` is `observed` when the cancel landed before `attached` and `inconclusive` when it landed after. An inconclusive trial verifies only the detach, no-shutdown, and fresh-subscription invariants.
