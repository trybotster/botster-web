...
The published `@trybotster/hub-test-support@0.1.8` artifact supplies conformance revision 15, including the generated targeted `read_mode_flags` request/response DTOs and ModeFlags scenarios, alongside the existing binary-safe history and typed readback/response-chunk contracts. ModeFlags is generated protocol scaffolding only; botster-web does not yet invoke the request or render a UI consumer. Shipped browser sessions invoke `read_screen` automatically when their exact subscription reaches `Attached`; live output received during that request is buffered FIFO so restoration cannot overwrite newer bytes. Empty screen text fabricates no history, and stale replies from detached or replaced attachment generations are discarded. `capture_snapshot` remains an explicit metadata helper and its opaque payload metadata is not a renderable terminal byte stream. Compatible hubs must still advertise `terminal_readback` and conformance fixture revision 14 or newer.
