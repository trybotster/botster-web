...
- Encrypted daemon requests, responses, and small lifecycle `daemon_event`s travel over the ordered control DataChannel after its Hello.
...
- Each terminal subscription uses one separate ordered DataChannel. Web creates it only after Hub reserves an opaque label and generation. The channel sends its own encrypted Hello before it carries Core terminal frames.
...
- `@trybotster/ui-contract@0.3.3` is the sole `UiNode`, action request/result, package-surface descriptor, supported-operation, entity-options projection, manifest-navigation, and package notice-reaction vocabulary source. Hub owns package admission and projects sanitized package/navigation rows plus `notice_reactions`. Host DTOs and revision-48 shared conformance fixtures come from `@trybotster/hub-test-support@0.1.43`. Core terminal types, binary input encoders, and feature tokens come from `@trybotster/terminal-protocol@0.3.0`. Web does not pin a Hub Git revision for terminal compatibility.
