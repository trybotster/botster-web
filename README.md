...
- Hub subscribe establishes the control channel only. The sessions surface explicitly subscribes to the `session` entity type, waits for its authoritative snapshot, and reconciles ordered pushed deltas. Every WebRTC generation gets a fresh subscription id; stale generations are discarded without `list_sessions`, SSE, polling, or legacy lifecycle refresh fallbacks.
...
This command builds the app, starts the package bridge in real-hub mode, starts an isolated local hub when `BOTSTER_HUB_BIN` is supplied, opens the compiled packaged UI in Playwright Chromium, and drives the mounted terminal data-plane path. Its default transport is the production WebRTC data plane. It proves revision-14 compatibility with required `terminal_readback`, package and session discovery, spawn of `botster-web-dogfood-session`, and two same-URL browser refresh and WebRTC reconnect cycles followed by explicit Attach to the existing session. On every attachment the shipped adapter automatically requests `read_screen`, installs its visible text into Restty, and only then flushes later live output. Snapshot/Scrollback `payload_base64` remains opaque engine state: the harness validates its `base64` encoding and byte count for subscription chronology but never renders or logs the payload as visible history. The harness also preserves mounted keyboard, resize, explicit `capture_snapshot`, response-chunk telemetry, deterministic exit, `process_exit`, and clean-shutdown proof. For the initial mount and each refresh attach, it separately asserts `Attaching -> optional opaque Snapshot/Scrollback -> Attached -> live terminal output` and visible ReadScreen restoration before buffered live output. The harness records the resolved hub and session-worker binary paths and versions when their Cargo manifests are discoverable. Each WebRTC ReadScreen restoration must report response-chunk telemetry with `total_bytes`, `chunk_count`, and `duration_ms`; duration must remain below the 10-second request deadline and at or below 8 seconds to preserve 2 seconds of headroom. Release acceptance repeats the full command five times and records all five redacted chronology/restoration summaries.

The retained terminal probes include `botster-web-dogfood-size:<rows>x<cols>` for resize and `botster-web-dogfood-exit` for deterministic process shutdown.

The published `@trybotster/hub-test-support@0.1.10` artifact supplies conformance revision 17, including generated `DaemonLocalWebrtcDeliveryChunk` v2 response/entity delivery, session lifecycle subscription fixtures, and targeted `read_mode_flags` scenarios alongside the binary-safe history and typed readback contracts. ModeFlags is generated protocol scaffolding only; botster-web does not yet invoke the request or render a UI consumer. Shipped browser sessions invoke `read_screen` automatically when their exact terminal subscription reaches `Attached`; live output received during that request is buffered FIFO so restoration cannot overwrite newer bytes. Empty screen text fabricates no history, and stale replies from detached or replaced attachment generations are discarded. `capture_snapshot` remains an explicit metadata helper and its opaque payload metadata is not a renderable terminal byte stream. Compatible hubs must still advertise `terminal_readback` and conformance fixture revision 14 or newer.
