...
Attach/Drain history is available on the shipped path today. The published `@trybotster/hub-test-support@0.1.3` artifact supplies the generated readback DTOs and late-attach fixture used by deterministic tests. Typed `read_screen` text and `capture_snapshot` metadata helpers are exposed only through the Playwright-injected live-harness controls; shipped browser sessions do not install or invoke them. Snapshot metadata (`rows`, `cols`, optional `payload_format`, and `payload_bytes`) is not a renderable terminal byte stream. `terminal_readback` therefore remains optional with a warning when absent, and conformance fixture revision 1 remains compatible.
