...
assert.match(resttyRenderer, /installTerminalInputCapture\(/);
assert.match(resttyRenderer, /this\.ptyTransport\.sendSemantic\(input\)/);
assert.match(resttyRenderer, /loadBinarySnapshot/);
assert.match(botsterTerminalPtyTransport, /sendSemantic\(input: TerminalSemanticInput\): boolean/);
assert.match(botsterTerminalPtyTransport, /onUncapturedInput/);
assert.doesNotMatch(botsterTerminalPtyTransport, /writeModeGatedInput|pendingSemantic/);
...
assert.match(smokeRealHubScript, /W-S5 negative control passed/);
assert.match(smokeRealHubScript, /in_page_reconnect: ablateReconnectClose \? "negative-control-passed" : "verified"/);
...
assert.match(liveProtocolHarnessScript, /terminalClosesBeforeFlood/);
assert.match(liveProtocolHarnessScript, /package-event flood closed the terminal subscription channel/);
assert.match(liveProtocolHarnessScript, /terminal_channel_closes:/);
assert.doesNotMatch(liveProtocolHarnessScript, /terminal delivery queue overflow|event flood overflowed the terminal delivery backlog/);
...
assert.doesNotMatch(terminal, /terminalLastRenderedOutput/);
assert.doesNotMatch(resttyRenderer, /terminalLastRenderedOutput/);
assert.match(terminal, /writeRawInput\(/);
...
assert.match(architecture, /After admission the channel carries binary chunks only/);
...
assert.match(resttyRenderer, /readOnly:\s*true/);
assert.match(resttyRenderer, /installTerminalInputCapture\(/);
assert.doesNotMatch(resttyRenderer, /pendingSemantic|takePendingSemantic|reportKind/);
...
assert.match(liveProtocolHarnessScript, /expected exactly one detach for subscription/);
...

{
  ...
  assert.equal(PINNED_REVISIONS.modular_restty.length, 40);
  assert.equal(PINNED_REVISIONS.modular_ghostty.length, 40);
  for (const file of ["internal.js", "chunk-hq13y361.js", "restty.js", "xterm.js"]) {
    const contents = await readFile(new URL(`../src/vendor/restty/${file}`, import.meta.url));
    assert.equal(createHash("sha256").update(contents).digest("hex").length, 64);
  }
}
